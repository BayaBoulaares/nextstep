...
// ✅ Importer depuis les types alignés backend
...
function useServices() {
  ...
  const fetch_ = React.useCallback(() => {
    ...
    setError(null)
    fetch("/api/services")
      .then(r => { if (!r.ok) throw new Error(`Erreur ${r.status}`); return r.json() as Promise<CloudServiceDTO[]> })
      .then(setServices)
...
export function ServicesTable({ isAdmin = false }: ServicesTableProps) {
  ...
  const columns: ColumnDef<CloudServiceDTO>[] = [
    ...
    {
      // ✅ CloudServiceDTO.cloudType (CloudType.java : PRIVÉ | PUBLIC | HYBRIDE)
