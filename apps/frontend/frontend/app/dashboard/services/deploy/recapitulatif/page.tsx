...
export default function RecapPage() {
  ...
  React.useEffect(() => {
    ...
    let d: DeploymentDraft
    try { d = JSON.parse(raw) } catch { setFetching(false); return }

    setDraft(d)
    if (!d.serviceId || !d.planId) { setFetching(false); return }
    ...
    getServiceById(d.serviceId)
      ...
      .catch(() => setLoadError("Impossible de charger les détails du service. Vérifiez votre connexion."))
      .finally(() => setFetching(false))
  }, [])
  ...
  if (loadError) {
    return (
      <div className="flex h-screen flex-col items-center justify-center gap-4">
        <p className="text-[13px] text-muted-foreground text-center max-w-xs">{loadError}</p>
        <div className="flex gap-3">
          <Button variant="outline" onClick={() => router.back()}>← Configuration</Button>
  ...
  if (!draft?.serviceId || !draft?.planId) {
    return (
      <div className="flex h-screen flex-col items-center justify-center gap-4 px-6">
        <p className="text-[13px] text-muted-foreground text-center">Aucune configuration trouvée.</p>
        <Button onClick={() => router.push("/dashboard/services")}>Retour au Marketplace</Button>
      </div>
    )
  }
        ...
        <div className="space-y-6">
          ...
          <RecapSection
            ...
            rows={[
              ...
              {
                label: "Ressource",
                value: <span className="font-mono text-[12px]">{draft.resourceName ?? "—"}</span>,
              },
              // OS uniquement pour les services VM
              ...(osLabel && requiresVm(category)
          ...
          <div className="flex items-center gap-3 pt-2">
            ...
            <Button
              className="h-9 text-[13px] font-medium bg-[#0a7fcf] hover:bg-[#0869b0] text-white flex-1"
              onClick={handleConfirm}
              disabled={loading || !plan || !draft.resourceName}
            >
              {loading
