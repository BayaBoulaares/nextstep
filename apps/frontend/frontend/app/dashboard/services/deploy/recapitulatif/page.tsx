...
//
//  1. Import OS_LABELS depuis @/lib/types
//  2. handleConfirm : ajout operatingSystem dans DeploymentRequest
//  3. Calcul prix : ht/tva/total gèrent le cas null (plan PAYG)
//  4. RecapSection "Service commandé" : ajout ligne OS
//  5. RecapSection "Configuration" : inchangée
//  6. Section "Détail tarifaire" : affichage différent selon isPayAsYouGo
//  7. Bandeau ⚠️ : message adapté si plan PAYG
//
...
export default function RecapPage() {
  ...
  const handleConfirm = async () => {
    ...
    const request: DeploymentRequest = {
      ...
      antiDdosEnabled:     draft.antiDdosEnabled   ?? false,
      additionalStorageGb: draft.additionalStorageGb ?? 0,
      // ⚠️ NOUVEAU : operatingSystem (enum) lu depuis le draft
  ...
  // ⚠️ MODIFICATION : plan PAYG → price est null → pas de calcul HT/TVA
  ...
  // Label OS lisible depuis le draft
        ...
        <div className="space-y-6">
          <RecapSection
            ...
            rows={[
              ...
              { label: "Specs",     value: plan ? planSpecs(plan) : "—"                             },
              { label: "Ressource", value: <span className="font-mono text-[12px]">{draft.resourceName ?? "—"}</span> },
              // ⚠️ NOUVEAU : ligne OS (uniquement si présent dans le draft)
          ...
          {/* ⚠️ MODIFICATION : section tarifaire adaptée PAYG vs prix fixe */}
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-[0.15em] text-muted-foreground mb-2">Détail tarifaire</p>
            <div className="border border-border rounded-xl overflow-hidden divide-y divide-border">
              {isPayg ? (
                // Plan PAYG — pas de montant fixe
                <>
          ...
          {/* ⚠️ MODIFICATION : message adapté selon PAYG */}
          <div className="flex items-start gap-2.5 border border-amber-200 bg-amber-50 rounded-xl px-4 py-3">
            <span className="shrink-0 mt-0.5">⚠️</span>
            <p className="text-[12px] text-amber-800 leading-relaxed">
