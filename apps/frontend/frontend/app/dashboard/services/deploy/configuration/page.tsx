...
import {
  OS_LABELS,
  CATEGORIES_WITHOUT_OS,
  CATEGORIES_WITHOUT_AS,
} from "@/lib/types"
import { cn } from "@/lib/utils"
...
function getOsOptions(category?: ServiceCategory): OperatingSystem[] {
  if (!category || CATEGORIES_WITHOUT_OS.includes(category)) return ["NONE"]
  return [
    "UBUNTU_24_04_LTS",
    "UBUNTU_22_04_LTS",
    "DEBIAN_12",
...
export default function ConfigurationPage() {
  ...
  const availabilitySetError = React.useMemo(() => {
    if (!availabilitySet) return null
    if (!/^[a-z][a-z0-9-]{0,61}[a-z0-9]$/.test(availabilitySet))
      return "Même format que le nom de ressource : minuscules, chiffres, tirets"
    return null
  }, [availabilitySet])
  ...
  React.useEffect(() => {
    ...
    const sid = draft?.serviceId
    const pid = draft?.planId
    if (!sid || !pid) { setFetching(false); return }
    ...
    getServiceById(sid)
      ...
      .catch(() => setLoadError("Impossible de charger le service. Vérifiez votre connexion."))
      .finally(() => setFetching(false))
  }, [])
  ...
  const handleContinue = () => {
    ...
    sessionStorage.setItem("deploy_draft", JSON.stringify({
      ...
      serviceId,
      planId,
      category,                                        // ← catégorie pour le routage
      resourceName,
      description,
      availabilityZone: zone,
      backupEnabled,
      operatingSystem: CATEGORIES_WITHOUT_OS.includes(category as ServiceCategory)
        ? "NONE"
        : operatingSystem,
      availabilitySet: availabilitySet || undefined,
    }))
    router.push("/dashboard/services/deploy/recapitulatif")
  }
  ...
  const showOs = !CATEGORIES_WITHOUT_OS.includes(category as ServiceCategory)
  const showAs = !CATEGORIES_WITHOUT_AS.includes(category as ServiceCategory)
        ...
        <div className="flex-1 min-w-0 space-y-5">
          ...
          {showOs && (
            <SectionCard icon="💿" title="Système d'exploitation" sub="Image de démarrage de la VM">
              <div className="space-y-1.5">
                <Label className="text-[12px]">
                  OS <span className="text-destructive">*</span>
                </Label>
                <Select
                  value={operatingSystem}
                  onValueChange={v => setOperatingSystem(v as OperatingSystem)}
          ...
          <SectionCard icon="🔧" title="Options" sub="Services additionnels">
            <div className="flex items-center justify-between py-1">
              ...
              <Switch
                checked={backupEnabled}
                onCheckedChange={setBackupEnabled}
                className="data-[state=checked]:bg-[#0a7fcf]"
              />
            </div>
          </SectionCard>

          {/* Availability Set — masqué pour le stockage */}
          {showAs && (
            <SectionCard
              icon="🔗"
              title="Haute disponibilité"
